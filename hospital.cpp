...
//function to sort the hospital data by numerating
...
string pateint_Contact[], int bookingCost[],
string hosptial_Name[], string locations[],
int beds[],
float rating[], string hospital_Contact[],
...

{
    ...
    PrintHospitalData(hospitals);
    PrintPatiendData(patients, hospitals);

    sortHospitalByName(hospitals);
    sortHospitalByRating(hospitals);
    //PrintHospitalBycity(hospitals);
    sortHospitalByBedsAvailable(hospitals);
    sortHospitalByBedsPrice(hospitals); }
    // Driver Code
     int main()
     {
    ...
    // Function Call
    HospitalManagement( patient_Name, patient_Id, patient_Contact, bookingCost, hospital_Name, locations, beds, ratings, hospital_Contact, doctor_Name, prices);
    return 0; }
